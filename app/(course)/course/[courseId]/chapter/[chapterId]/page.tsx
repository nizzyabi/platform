    ...
    <div className="w-full px-5 lg:pr-12 overflow-hidden mb-5">
      ...
      {chapter.code && chapter.code.trim().length > 0 && (
        <div>
          ...
          <div>
            <SyntaxHighlighter
              className=" border-slate-100/20 rounded-[5px] shadow-lg shadow-black border-2"
      ...
      <div className="flex items-center justify-center space-x-8 pt-8">
        {links.map((link, index) => (
          <Link
            href={link.href}
            key={link.href}
            className="bg-slate-100 w-[100px] py-2 rounded-[5px] hover:opacity-75 duration-300 border border-slate-100/20"
          >
            <div className="flex item-center justify-center">
              <p className="text-center text-5xl">{link.icon}</p>
            </div>
            <p className="text-center text-[#2e2e2e] font-semibold mt-2">
              {link.label}
            </p>
          </Link>
        ))}
