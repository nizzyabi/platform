...
const formSchema = z.object({
  notionLink: z.string().min(1, {
    message: 'Notion link is required'
  })
})

export const NotionForm = ({ initialData, courseId }: NotionLinkFormProps) => {
  // state for editing
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      notionLink: initialData?.notionLink || ''
    }
  })
  // destructure
  ...
  return (
    <div className="mt-6 border border-slate-100/20 shadow-md bg-[#131212] bg-opacity-95 rounded-xl p-4">
      ...
      {!isEditing && <p className="text-lg mt-2">{initialData.notionLink}</p>}
      {isEditing && (
        <Form {...form}>
          <form
